...
with open("input15.txt") as f:
	for l in f.readlines():
		...
		ingr[ingredient[:-1]] = (int(capacity[:-1]), int(durability[:-1]), int(flavour[:-1]), int(texture[:-1]), int(calories[:-1]))
		igrs.append((int(capacity[:-1]), int(durability[:-1]), int(flavour[:-1]), int(texture[:-1]), int(calories[:-1])))
maxscore = 0
n = 100
for i in range(1, n+1):
	for j in range(1, n+1-i):
		for k in range(1, n+1-i-j):
			for l in range(1, n+1-i-j-k):
				if i + j + k + l != 100:
					continue
